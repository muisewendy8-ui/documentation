...
.. important::
   ...
   reflected in the database**. This means that employees who check-in during onboarding training
   are logged into work in the **Attendances** app.
...
dashboard loads, with all employees listed in a default Kanban view. All Kanban cards display either
a red :guilabel:`Absent` or a green :guilabel:`On Site` text in the upper-right corner of the card,
indicating the current status for the employee.
...
After approximately seven seconds, the screen reverts to the main *Onboarding* dashboard, or to exit
the screen sooner, click the :guilabel:`OK` button.
...
dashboard loads displaying two options: :guilabel:`Scan your badge`, or :guilabel:`New Set-up
(Employee/Badge)`.
...
and a *Set-up* pop-up window loads. Using the drop-down menu beneath the text :guilabel:`Select
...
the button above the text :guilabel:`RFID Token with reader on tablet`, and a dashboard loads
displaying two options: :guilabel:`Scan your badge`, or :guilabel:`New Set-up (Employee/Badge)`.
...
.. important::
   ...
   manufacturer's directions to install the RFID reader, and set up the RFID key fob. Follow the
