...
kit <amazon_connector/fba/product-kit>` in Odoo based on the original Amazon product. When a new FBA
order syncs, Odoo sells the product kit, which prevents the error that occurs with the tracked
product.
...
.. note::
   ...
   Keeping this link active causes a synchronization failure for every Amazon FBA order. By
