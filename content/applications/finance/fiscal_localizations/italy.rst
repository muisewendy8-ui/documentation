...
.. list-table::
   ...
   * - Italy - Italy - E-invoicing - Additional module to support the debit notes (nota di debito -
       NDD)
     - `l10n_it_edi_ndd`
     - Support for debit notes (nota di debito - NDD) by adding payment methods and document types
   * - Italy - Italy - E-invoicing - Bridge module between Italy NDD and Account Debit Note
     - `l10n_it_edi_ndd_account_dn`
     - Support for debit notes (nota di debito - NDD) by adding debit note fields
...
**reverse charge** taxes. These tax grids are identifiable by the :ref:`VJ
<localizations/italy/grids>` tag, and can be found by going to :menuselection:`Accounting -->
