...
.. note::
   The Türkiye chart of accounts is based on the GİB’s 7/A accounts, which provide a baseline of the
   necessary accounts. 7/B accounts are archived and can be unarchived if required.
...
.. warning::
   Do not delete taxes, as they are used for the e-invoice and e-archive invoices. However, new
   taxes can be generated, such as :ref:`withholding tax reasons
   <localizations/turkiye/tax-creation>` that do not exist yet in the database.
...
The Turkish localization ensures compliance with the Turkish Revenue Administration (GİB)
requirements for e-invoice, e-archive, e-dispatch, and e-ledger processes.
...
- :guilabel:`Company Name`: must match GİB registration
...
.. note::
   - The :guilabel:`eInvoice format` is set automatically based on the contact's country. To edit
     this field, enter :doc:`developer mode <../../general/developer_mode>` to display the field and
...
#. Enable :doc:`developer mode <../../general/developer_mode>` and navigate to
   :menuselection:`Accounting --> Configuration --> GIB Codes`.
...
Odoo integrates with Nilvera, a GİB-approved electronic document intermediary, to manage e-invoice,
e-archive, e-dispatch, and e-ledger processes.

The integration handles:

- XML data generation in a GİB-compliant format
- secure transmission to Nilvera
- sychronization of documents and statuses within Odoo.
...
#. When :ref:`creating a customer invoice <accounting/invoice/creation>`, after selecting a
   ...
   - if the customer's :guilabel:`Nilvera Status` is :guilabel:`e-Invoice`, one field will appear:

     - :guilabel:`Invoice Scenario`: the scenario of the invoice to be sent to GİB
...
#. From an invoice form view

   #. Once the :guilabel:`Nilvera Status` is :guilabel:`Successful`, a new button labeled
      :guilabel:`Fetch Nilvera PDF` appears.
...
#. Open an existing delivery order or create a new one. Once a :guilabel:`Delivery Address` is
   selected, the :guilabel:`e-Dispatch` tab will appear.
#. Fill in the necessary information in the :guilabel:`e-Dispatch` tab.
#. Once the delivery order is validated, the :guilabel:`Generate GİB e-Dispatch (XML)` button will
   appear. Click the :guilabel:`Generate GİB e-Dispatch (XML)` button to generate the XML file. The
   generated file will be attached to the delivery order.
#. Download the generated file and upload it to Nilvera for processing.
...
with these requirements is essential for the legal validity of e-Documents issued through the
Turkish Revenue Administration (GİB) via Nilvera.
